...
| Tool | Purpose |
|---|---|
| **Vitest** | Unit tests + coverage (auto-ratchet at 80%) |
...
Every UI component change produces:

1. **Component** (`.tsx`)
2. **Unit test** (`.test.tsx`) — Vitest + Testing Library
3. **Story** (`.stories.tsx`) — Storybook
4. **E2E test** (`e2e/*.spec.ts`) — Playwright
5. **Screenshot** — auto-generated by Playwright

This rule is documented in AGENTS.md, CLAUDE.md, .cursorrules, and .github/copilot-instructions.md in every scaffolded project.
